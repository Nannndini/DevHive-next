...
export function DocumentViewer({ documentId, activeChunkId, onClose }: DocumentViewerProps) {
  ...
  useEffect(() => {
    async function fetchDocument() {
      ...
      // Mock data fetch
      setTimeout(() => {
        setDocName("mock_document.pdf")
        setChunks([
          { id: "1", content: "This is a simulated document chunk.", metadata: { index: 0 } },
          { id: "2", content: "It shows the integration of AI context.", metadata: { index: 1 } }
        ])
        setLoading(false)
      }, 500)
    }
