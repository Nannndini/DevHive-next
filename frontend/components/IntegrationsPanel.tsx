...
export function IntegrationsPanel() {
  ...
  useEffect(() => {
    // Mock connections
    setConnections([
      { id: "1", platform_name: "Notion", base_url: "https://api.notion.com", platform_type: "notion", status: "active", last_synced_at: new Date().toISOString() }
    ])
  }, [])
  ...
  const handleSync = async (integrationId: string) => {
    ...
    try {
      ...
      setConnections(prev => prev.map(c => c.id === integrationId ? { ...c, last_synced_at: new Date().toISOString() } : c))
    } catch (err: any) {
      console.error(err)
